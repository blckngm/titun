...
#[derive(Debug)]
pub struct RecvFromAsync<'a, 'b> {
    socket: &'a UdpSocket,
    buf: &'b mut [u8]
}

impl<'a, 'b> Future for RecvFromAsync<'a, 'b> {
    type Item = (usize, SocketAddr);
    type Error = io::Error;

    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        self.socket.poll_recv_from(self.buf)
    }
}

#[must_use = "futures do nothing unless polled"]
#[derive(Debug)]
pub struct SendToAsync<'a, 'b> {
    socket: &'a UdpSocket,
    destination: SocketAddr,
    buf: &'b [u8],
}

impl <'a, 'b> Future for SendToAsync<'a, 'b> {
    type Item = usize;
    type Error = io::Error;

    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        self.socket.poll_send_to(self.buf, &self.destination)
    }
}
...
impl UdpSocket {
    ...
    /// Ordinary `sendto`. Does not register on reactor.
    pub fn send_to(&self, buf: &[u8], target: impl Into<SocketAddr>) -> io::Result<usize> {
        self.io.get_ref().send_to(buf, &target.into())
    }
    ...
    /// Send to async.
    pub fn send_to_async<'a, 'b>(&'a self, buf: &'b [u8], target: impl Into<SocketAddr>) -> SendToAsync<'a, 'b> {
        SendToAsync {
            socket: self,
    ...
    pub fn recv_from_async<'a, 'b>(&'a self, buf: &'b mut [u8]) -> RecvFromAsync<'a, 'b> {
        RecvFromAsync {
            socket: self,
            buf,
        }
