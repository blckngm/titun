...
ip netns exec titun-test-1 taskset -c 0,1 titun -c tun1.conf tun1
# ip netns exec titun-test-1 ip link add tun1 type wireguard

ip netns exec titun-test-2 taskset -c 4,5 titun -c tun2.conf tun2
...
ip netns exec titun-test-1 taskset -c 2 iperf3 -s 2>&1 > /dev/null &
# ip netns exec titun-test-1 iperf3 -p 5202 -s 2>&1 > /dev/null &
sleep 1
ip netns exec titun-test-2 taskset -c 3 iperf3 -c 192.168.77.1
